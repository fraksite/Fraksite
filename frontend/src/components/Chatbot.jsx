...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { sender: 'bot', text: 'Halo! Ada yang bisa kami bantu terkait layanan Fraksite?' }
  ]);
    ...
    try {
      ...
      if (selectedModel === 'openrouter/free') {
        ...
        try {
          ...
          if (error.status === 429) {
            console.warn('Model terkena rate limit. Mencoba OpenRouter Free Router...');
            data = await sendToOpenRouter('openrouter/free', apiMessages);
          } else {
            throw error;
          }
      ...
      if (error.status === 429) {
        errorText = 'Maaf, layanan AI gratis sedang mencapai batas penggunaan.\n\nSilakan tunggu beberapa saat kemudian coba lagi.';
      } else if (error.status === 401) {
        errorText = 'API Key OpenRouter tidak valid.\n\nSilakan periksa API Key pada file .env.';
      } else if (error.status === 402) {
        errorText = 'Akun OpenRouter tidak memiliki saldo atau akses yang cukup untuk request ini.';
      } else if (error.status === 403) {
        errorText = 'Request ditolak oleh OpenRouter.\n\nPeriksa API Key dan konfigurasi akun OpenRouter.';
      } else if (error.status === 404) {
        errorText = 'Model atau endpoint OpenRouter tidak ditemukan.\n\nCoba pilih model Auto Free.';
      } else {
        ...
        <div className="bg-white w-80 h-[28rem] shadow-2xl rounded-2xl border border-gray-200 flex flex-col mb-4 overflow-hidden">
          ...
          <div className="bg-gray-900 text-white p-3 flex flex-col gap-2">
            ...
            <select
              ...
              disabled={isLoading}
              className="w-full bg-gray-800 text-xs text-gray-200 border border-gray-700 rounded p-2 outline-none focus:border-gray-500 disabled:opacity-50"
            >
      ...
      {!isOpen && (
        ...
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01 M12 10h.01 M16 10h.01 M9 16H5 a2 2 0 01-2-2V6 a2 2 0 012-2h14 a2 2 0 012 2v8 a2 2 0 01-2 2h-5 l-5 5v-5z" />
          </svg>
        </button>
      )}
    </div>
